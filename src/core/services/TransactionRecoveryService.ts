import { TransactionAttempt, TransactionStatus, ITransactionAttempt } from '../../models/TransactionAttempt';
...
export class TransactionRecoveryService {
   ...
   */
  public async markTransactionComplete(
   ...
   */
  public async markTransactionFailed(
    transactionId: string,
    error: string
  ): Promise<void> {
    const transaction = await TransactionAttempt.findById(transactionId);
  ...
  private async retryTransaction(transaction: ITransactionAttempt): Promise<void> {
    try {
      ...
      logger.info(`Retrying transaction ${transaction._id}, attempt ${transaction.retryCount} of ${transaction.maxRetries}`);
      ...
      if (!plan.isActive) {
        logger.info(`Plan ${transaction.planId} is no longer active, cancelling recovery for transaction ${transaction._id}`);
        transaction.status = TransactionStatus.FAILED;
      ...
      logger.info(`Executing recovered transaction ${transaction._id} for amount ${transaction.amount}`);
      ...
      if (transaction.retryCount >= transaction.maxRetries) {
        logger.error(`Transaction ${transaction._id} has exceeded max retries (${transaction.maxRetries}), marking as permanently failed`);
        ...
        const backoffMinutes = Math.min(Math.pow(2, transaction.retryCount), this.maxRetryDelayMinutes);
        transaction.lastAttemptTime = new Date(Date.now() + backoffMinutes * 60 * 1000);
        
        await transaction.save();
        logger.info(`Scheduled next retry for transaction ${transaction._id} in ${backoffMinutes} minutes`);
      }
    }
  }
